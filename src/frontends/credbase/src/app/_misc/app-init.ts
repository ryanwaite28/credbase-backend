...
//         .pipe(take(1))
//         .pipe(map((state) => {
//           socketEventsService.registerAppEventListenerStreams(event_types_map);
//           return state;
//         }));
  ...
  function APP_INITIALIZER_FN(
    ...
    const promise = clientService.getCsrfToken()
      .pipe(
        ...
        //     .pipe(take(1))
        //     .pipe(map((state) => {
        //       socketEventsService.registerAppEventListenerStreams(COMMON_EVENT_TYPES);
        //       return state;
        //     }));
